...
type ReplyDraft = { content?: string; characterId?: string };
...
export function ForumReplyEditor({ boardSlug, topicSlug, topicId, isRoleplay, characters }: { boardSlug: string; topicSlug: string; topicId: string; isRoleplay: boolean; characters: CharacterOption[] }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    void loadForumDraft<ReplyDraft>(draftKey).then((draft) => {
      if (!active) return;
      if (draft?.content) {
        setContent(draft.content.slice(0, 50000));
        if (isRoleplay && draft.characterId && characters.some((character) => character.id === draft.characterId)) setCharacterId(draft.characterId);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      if (!content.trim() && !characterId) {
        void deleteForumDraft(draftKey).then((ok) => { if (ok) setDraftStatus(null); });
        return;
      }
      setDraftStatus("saving");
      void saveForumDraft(draftKey, { content, characterId }).then((ok) => setDraftStatus(ok ? "saved" : null));
    }, 1200);
    return () => window.clearTimeout(timer);
  }, [characterId, content, draftKey, draftReady]);
  ...
  return <form action={createForumPost}>
    ...
    <div hidden={previewOpen}>
      ...
      <div className="forum-reply-box__footer"><span>{content.length.toLocaleString("fr-FR")} / 50 000{draftLabel ? ` · ${draftLabel}` : ""}</span><div className="forum-reply-box__actions"><button className="button button--ghost button--small" type="button" onClick={() => setPreviewOpen(true)}>Prévisualiser</button><ReplyButton disabled={!canPublish}/></div></div>
    </div>
