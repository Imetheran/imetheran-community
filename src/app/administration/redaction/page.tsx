...
function communityRoleLabel(role: MemberRow["member_role"]) {
...
export default async function AdminEditorialAccessPage({
  ...
  return (
    <main className="site-shell admin-page admin-editorial-access-page">
      <SiteHeader />
      ...
      <section className="admin-hero">
        <div className="content-frame admin-hero__layout">
          <div>
            ...
            <p>Choisissez qui peut écrire ou relire les Chroniques et Gazettes.</p>
          </div>
      ...
      <section className="content-frame admin-workspace">
        ...
        <div className="admin-metrics admin-editorial-access-metrics" aria-label="Résumé des accès rédaction">
        ...
        <section className="admin-panel admin-editorial-access-panel">
          <header className="admin-panel__head">
            <div><p className="eyebrow">Permissions</p><h2>Accès des membres</h2></div>
            <span className="admin-panel__status">{filteredMembers.length} membre{filteredMembers.length > 1 ? "s" : ""}</span>
          </header>

          <form className="admin-cms-filters admin-editorial-access-search" method="get">
          ...
          {filteredMembers.length ? (
            <div className="admin-member-cards admin-editorial-access-list">
              {filteredMembers.map((member) => {
                const permission = permissionMap.get(member.user_id);
                const isAdmin = member.member_role === "admin";

                return (
                  <article className="admin-member-card admin-editorial-access-card" key={member.user_id}>
                    <header className="admin-member-card__identity">
                      ...
                      <div className="admin-member-card__badges">
                        <span className={`admin-member-role admin-member-role--${member.member_role}`}>{communityRoleLabel(member.member_role)}</span>
                        <span className={permission || isAdmin ? "is-confirmed" : "is-unconfirmed"}>
                          {permission ? cmsRoleLabels[permission.cms_role] : isAdmin ? "Accès complet" : "Sans accès CMS"}
                        </span>
                      </div>
                    </header>

                    {isAdmin ? (
                      <div className="admin-cms-access-admin">
                        <span aria-hidden="true">✦</span>
                        <strong>Accès CMS complet</strong>
                        <small>Chroniques et Gazettes</small>
                      </div>
                    ) : (
                      <form className="admin-cms-access-form" action={setCmsPermission}>
                        <input type="hidden" name="user_id" value={member.user_id} />

                        <label className="admin-cms-access-role">
                          <span>Rôle CMS</span>
                        ...
                        <fieldset className="admin-cms-access-scope">
                          <legend>Contenus autorisés</legend>
                          <label className="admin-cms-access-toggle">
                            <input type="checkbox" name="can_chronicles" defaultChecked={permission?.can_chronicles ?? false} />
                            <span><strong>Chroniques</strong><small>Récits et campagnes</small></span>
                          </label>
                          <label className="admin-cms-access-toggle">
                            <input type="checkbox" name="can_gazettes" defaultChecked={permission?.can_gazettes ?? false} />
                            <span><strong>Gazettes</strong><small>Numéros et articles</small></span>
                          </label>
                        </fieldset>
            ...
            <div className="admin-empty-state">
              <strong>Aucun membre ne correspond à la recherche.</strong>
              <Link className="text-link" href="/administration/redaction">Réinitialiser →</Link>
            </div>
          )}
        </section>
      </section>
