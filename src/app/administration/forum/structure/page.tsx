...
export default async function ForumStructurePage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  const [sectionsResult, boardsResult] = await Promise.all([
  ...
  const loadError = sectionsResult.error || boardsResult.error;
      ...
      <section className="admin-hero">
        <div className="content-frame admin-hero__layout">
          <div>
            ...
            <p>Organisez les catégories, archivez une campagne et ajustez les droits de lecture, création et réponse sans modifier le code du site.</p>
          </div>
                  ...
                  <div className={styles.boardList}>
                    {sectionBoards.map((board) => (
                      <article className={`${styles.boardCard}${board.is_active ? "" : ` ${styles.archived}`}`} key={board.id}>
                        <header><div><div className={styles.badges}><span>{policyLabel(board.topic_creation)} · sujets</span><span>{policyLabel(board.reply_policy)} · réponses</span>{!board.is_active ? <span>Archivé</span> : null}</div><h4>{board.title}</h4><p>{board.description || "Aucune description."}</p></div><code>{board.slug}</code></header>
