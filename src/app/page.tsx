...
export default async function Home() {
  ...
  const [topicsResult, boardsResult, chronicleResult, openChroniclesResult, charactersResult] = await Promise.all([
    supabase
      .from("forum_topics")
      .select("id, board_id, title, slug, last_activity_at, post_count")
  ...
  return (
    <main className="site-shell">
          ...
          <div className="activity-layout">
            <article className="panel panel--topics">
              <div className="panel__heading">
                ...
                <span className="status-pill status-pill--quiet">Forum connecté</span>
              </div>
              ...
              {topics.length ? (
                <div className="home-live-topic-list">
                  {topics.map((topic) => {
                    ...
                    return (
                      <Link className="home-live-topic" href={href} key={topic.id}>
                        <span className="home-live-topic__mark" aria-hidden="true">◆</span>
                        <span className="home-live-topic__copy"><strong>{topic.title}</strong><small>{board?.title ?? "Forum"} · {topic.post_count} message{topic.post_count > 1 ? "s" : ""}</small></span>
            ...
            <aside className="activity-sidebar" aria-label="Informations à retenir">
              <article className="panel notice-card">
                ...
                <p>Le forum, les fiches de personnages, les liens et les chroniques partagent désormais le même socle communautaire.</p>
              ...
              <article className="panel compact-status">
                <div><span className="status-dot" aria-hidden="true" /><span>Chroniques ouvertes</span><strong>{openChronicleCount}</strong></div>
      ...
      <section className="home-section home-section--gazette" aria-labelledby="gazette-title">
        <div className="content-frame">
          ...
          <article className="gazette-feature">
            <div className="gazette-cover" style={{ backgroundImage: `linear-gradient(180deg, rgba(7,7,7,.08), rgba(7,7,7,.66)), url(${featuredGazette.coverImage})` }} aria-hidden="true">
              <div className="gazette-cover__topline"><span>Édition {String(featuredGazette.issueNumber).padStart(2, "0")}</span><span>{formatPublicationDate(featuredGazette.publishedAt)}</span></div>
            ...
            <div className="gazette-feature__story">
              <div className="gazette-feature__meta"><span className="status-pill">Publié</span><span>{featuredGazette.edition}</span></div>
              ...
              <div className="gazette-feature__contents"><span>Dans ce numéro</span><ul>{featuredGazette.highlights.map((highlight) => <li key={highlight}>{highlight}</li>)}</ul></div>
              <div className="gazette-feature__actions"><Link className="button button--primary" href={`/gazettes#${featuredGazette.slug}`}>Lire la gazette</Link><span className="gazette-feature__cms-note">Numéro d’essai · contenu provisoire</span></div>
            </div>
          </article>
        </div>
      </section>
      ...
      <section className="home-section home-section--chronicles" aria-labelledby="chronicles-title">
        <div className="content-frame">
          ...
          {featuredChronicle ? (
            ...
            <div className="empty-feed"><span className="empty-feed__mark" aria-hidden="true">✦</span><div><strong>Aucune chronique publiée.</strong><p>La première chronique mise en ligne depuis le CMS apparaîtra automatiquement ici.</p></div></div>
          )}
        </div>
      </section>
      ...
      <section className="home-section home-section--forum" aria-labelledby="forum-title">
        <div className="content-frame">
          <header className="section-heading section-heading--center"><p className="eyebrow">Place publique</p><h2 id="forum-title">Le forum</h2><p>Des espaces hors-RP pour la communauté et la préparation du jeu, puis des sections RP dédiées aux scènes et campagnes.</p></header>
          <div className="forum-grid">{forumCategories.map((category, index) => <Link className="forum-category" href="/forum" key={category.title}><span className="forum-category__index">{String(index + 1).padStart(2, "0")}</span><span className="forum-category__content"><small>{category.meta}</small><strong>{category.title}</strong><span>{category.description}</span></span><span className="forum-category__arrow" aria-hidden="true">→</span></Link>)}</div>
        </div>
      </section>

      <section className="home-section home-section--characters" aria-labelledby="characters-title">
        <div className="content-frame">
          <header className="section-heading section-heading--row"><div><p className="eyebrow">Carnet de rencontres</p><h2 id="characters-title">Personnages à découvrir</h2></div><Link className="text-link" href="/personnages">Tous les personnages <span aria-hidden="true">→</span></Link></header>
          ...
          {liveCharacters.length ? (
              ...
              {liveCharacters.map((character) => {
                ...
                return (
                  <Link className="character-card" href={`/personnages/${character.slug}`} key={character.id}>
                    <div className="character-card__portrait" aria-hidden="true"><span>{initials(character.name)}</span></div>
                    <div className="character-card__body"><small>{character.people || "Personnage"}{character.world ? ` · ${character.world}` : ""}</small><h3>{character.name}</h3><p className="character-card__epithet">{character.epithet || "Personnage d’Imetheran"}</p><p>{character.short_summary || "Fiche communautaire publiée."}</p><div className="character-card__tags">{traits.slice(0, 3).map((trait: string) => <span key={trait}>{trait}</span>)}</div></div>
            ...
            <div className="empty-feed"><span className="empty-feed__mark" aria-hidden="true">◎</span><div><strong>Aucun personnage public pour le moment.</strong><p>Les fiches publiées par les membres apparaîtront ici automatiquement.</p></div></div>
          )}
        </div>
      </section>
      ...
      <section className="home-section home-section--editorial" aria-labelledby="editorial-title">
        <div className="content-frame">
          ...
          <div className="editorial-grid">{editorialItems.map((item) => <Link className="editorial-card" href={item.href} key={item.title}><small>{item.kicker}</small><h3>{item.title}</h3><p>{item.text}</p><span>Découvrir <span aria-hidden="true">→</span></span></Link>)}</div>
        </div>
      </section>

      <section id="decouvrir" className="home-section home-section--about" aria-labelledby="about-title">
        <div className="content-frame about-strip"><div><p className="eyebrow">Notre foyer</p><h2 id="about-title">Bienvenue sur Imetheran</h2></div><p>Une communauté francophone Final Fantasy XIV consacrée au rôleplay, aux récits et aux personnages. Un lieu pensé pour retrouver l’esprit des forums d’autrefois avec des outils communautaires modernes.</p><strong>À l’ancienne, comme autrefois.</strong></div>
      </section>
