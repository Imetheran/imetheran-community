...
export default async function Home() {
  ...
  const [
    ...
    supabase.from("characters").select("id", { count: "exact", head: true }).eq("status", "published").eq("visibility", "public").eq("is_moderation_hidden", false),
  ]);
  ...
  const publishedGazetteCount = publishedGazettesResult.count ?? 0;
  const publicCharacterCount = publicCharactersResult.count ?? 0;
        ...
        <div className="content-frame">
          ...
          <div className="activity-layout">
            <article className="panel panel--topics">
              ...
              })}</div> : <div className="empty-feed"><span className="empty-feed__mark" aria-hidden="true">✦</span><div><strong>Aucun sujet visible pour le moment.</strong><p>Les discussions accessibles à votre session apparaîtront ici dès qu’elles seront publiées.</p></div></div>}
            </article>
            <aside className="activity-sidebar" aria-label="Informations à retenir">
              <article className="panel notice-card"><p className="panel__kicker">À retenir</p><h3>Bienvenue sur Imetheran</h3><p>Retrouvez au même endroit les discussions, personnages, relations, Chroniques et Gazettes de la communauté.</p><Link className="text-link" href="/forum">Découvrir le forum <span aria-hidden="true">→</span></Link></article>
          ...
          {featuredGazette ? (
            <article className="gazette-feature">
              <div className="gazette-cover" style={featuredGazette.cover_image ? { backgroundImage: `linear-gradient(180deg, rgba(7,7,7,.08), rgba(7,7,7,.66)), url(${featuredGazette.cover_image})` } : { backgroundImage: "linear-gradient(180deg, rgba(7,7,7,.18), rgba(7,7,7,.7)), var(--hero-image)" }} aria-hidden="true">
                ...
                <div className="gazette-cover__masthead">{featuredGazette.title}</div><div className="gazette-cover__headline">{featuredGazette.headline}</div><span className="gazette-cover__demo">À la une</span>
              </div>
              <div className="gazette-feature__story">
                ...
                <div className="gazette-feature__actions"><Link className="button button--primary" href={`/gazettes/${featuredGazette.slug}`}>Lire la Gazette</Link><span className="gazette-feature__cms-note">Numéro à la une</span></div>
              </div>
            </article>
          ) : <div className="empty-feed"><span className="empty-feed__mark" aria-hidden="true">✦</span><div><strong>Aucune Gazette publiée.</strong><p>Le prochain numéro publié apparaîtra automatiquement ici.</p></div></div>}
        </div>
      </section>
